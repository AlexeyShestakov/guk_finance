...
trait Item {
    ...
    static public function exceptionIfClassNotCompatibleWithTree($class_name){
        $global_class_name = \Cebera\Model\Helper::globalizeClassName($class_name);
        ...
        \Cebera\Model\Helper::exceptionIfClassNotImplementsInterface($global_class_name, 'Cebera\Model\InterfaceLoad');
        \Cebera\Model\Helper::exceptionIfClassNotImplementsInterface($global_class_name, 'Cebera\Model\InterfaceFactory');
    }
    ...
    public function setParentId($new_parent_id){
        ...
        $this->parent_id = $new_parent_id;
        \Cebera\DB\DBWrapper::query(self::DB_ID, 'update ' . self::DB_TABLE_NAME . ' set parent_id = ? where id = ?',
    ...
    public function setWeight($weight){
        ...
        $this->weight = $weight;
        \Cebera\DB\DBWrapper::query(self::DB_ID, 'update ' . self::DB_TABLE_NAME . ' set weight = ? where id = ?',
    ...
    protected function loadChildrenIdsArr(){
        ...
        $this->children_item_ids_arr = \Cebera\DB\DBWrapper::readColumn(self::DB_ID, $sql, array($parent_id));
    }
    ...
    public static function getRootItemsIdsArr()
    {
        ...
        $item_id_arr = \Cebera\Cache\CacheWrapper::get($cache_key);
        ...
        $item_id_arr = \Cebera\DB\DBWrapper::readColumn(self::DB_ID, $sql);

        \Cebera\Cache\CacheWrapper::set($cache_key, $item_id_arr, 3600);
