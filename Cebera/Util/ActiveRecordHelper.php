...
class ActiveRecordHelper
{
    ...
    public static function saveModelObj($model_obj)
    {
        ...
        if ($model_id_value == '') {
            ...
            $last_insert_id = \Cebera\DB\DBWrapper::lastInsertId($db_id, $db_sequence_name);
            $property_obj->setValue($model_obj, $last_insert_id);

            \Cebera\Logger\Logger::logObjectEvent($model_obj, 'CREATE');
        } else {
            ...
            \Cebera\DB\DBWrapper::query($db_id, $query, $values_arr);

            \Cebera\Logger\Logger::logObjectEvent($model_obj, 'UPDATE');
        }
    }
    ...
    public static function loadModelObj($model_obj, $id)
    {
        ...
        $data_obj = \Cebera\DB\DBWrapper::readObject(
            $db_id,
    ...
    public static function deleteModelObj($model_obj)
    {
        ...
        $result = \Cebera\DB\DBWrapper::query(
            $db_id,
            'DELETE FROM ' . $db_table_name . ' where ' . $db_id_field_name . ' = ?',
            array($model_id_value)
        );

        \Cebera\Logger\Logger::logObjectEvent($model_obj, 'DELETE');
